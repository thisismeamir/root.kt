...
/**
 * Walks all [TKey] records in this [ByteBuffer] sequentially from [begin] to [end],
 * returning them as an ordered list.
 *
 * ROOT files are a flat sequence of consecutive data records. Each record is exactly
 * [TKey.nbytes] bytes long, so the next record always starts at the current
 * offset + [TKey.nbytes]. This function exploits that invariant to traverse the
 ...
 *
 * The walk stops early if a non-positive [TKey.nbytes] is encountered. A negative
 ...
 * Callers that want only user data should filter by [TKey.className].
 *
 ...
 * @return A list of all [TKey] records found between [begin] and [end], in file order.
 */
fun ByteBuffer.walkKeys(begin: Int, end: Long): List<TKey> {
    val keys = mutableListOf<TKey>()
